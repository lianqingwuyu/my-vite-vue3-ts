...
const publicKey = '042ccb0a246b6ccb7776fb911939557315b434806f9b2a0ba49f24700dd4ab211f32adc1d552ff3f7347b2021ed888eb9fca6c69567ba68d8b99688a5805e343c3'
const privateKey = '087cab12a9653da5c3c3d56e32c586d0b59247f9dba479b92e37273f5ad56a85'
// 设置请求超时
...
instance.defaults.headers.common['Authorization'] = 'Basic c2FiZXI6c2FiZXJfc2VjcmV0'
...
instance.interceptors.request.use(config => {
    if (isCrypto) {
        ...
        const encryptStr = sm2.doEncrypt(paramStr, publicKey, 1)
        config.data = {}
        config.params = {}
...
instance.interceptors.response.use(response => {
    ...
    if (isCrypto) {
        data = response.data;
        response.data = JSON.parse(sm2.doDecrypt(data.substr(2, data.length), privateKey))
    }
    data = response.data;
    // 对响应数据做点什么
    ...
    if (response) {
        ...
        if (error.message.includes('timeout')) {
            // ElMessage Box.alert('请求超时，请稍后重试', '提示', {
            //   lockScroll: true,
...
const errorHandle = (status, other) => {
    ...
    switch (status) {
        ...
        // 403:  权限不足，拒绝访问
        case 403:
            // ElMessage Box.alert('权限不足，拒绝访问', '提示');
            break;
        // 404: 请求不存在
        case 404:
            // ElMessage Box.alert('请求的资源不存在或请求地址出错', '提示');
            break;
        // 500: 服务器错误
        case 500:
            ElMessage.error('服务器错误,系统异常');
            //ElMessage Box.alert('服务器错误', '提示');
            break;
        // 1000001: token 异常
        case 1000001:
            // ElMessage Box.alert("token 异常，请联系管理员", '提示');
            break;
        default:
